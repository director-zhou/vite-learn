...
通过`esbuild`构建完成之后,在默认缓存文件中会生成收集依赖构建后的文件
...
不但要对收集好的依赖文件进行`esbuild`构建，同时还需要给每个依赖添加载时的寻找映射路径和一些是否需要强制操作的信息映射。
`Vite`将依赖元信息存放在元信息的`data`的`optimized`对象内。
...
需要对每个依赖都要进行生成元信息，通过`for in`循环对每个依赖生成元信息,将生成以下内容:
...
`needsInterop`是一个`Boolean`值,如果构建的入口文件采用`esm`的方式，则为`false`, 如果是`umd`或者`cjs`则为`true`, `cjs`的引入模块方式会引起`esbuild`构建结果有差异, 这种差异是因为`esm`与`cjs`语法进行混用导致的结果。
...
前面是`entry.js`通过`esm`方式进行导出，在`main`中通地`cjs`的方式进行混用导入导出
...
打开`network`找到`main.js`文件可以发现,原本的引入方式,在服务端已经被`importAnalysis`插件中的`transfom`进行了转换。以为转换结果:
...
对于`cjs`导出模块`dayjs`做了一层值的判断，如果导出对象上有`__esModule`,最终结果是返回引入模块对象上的`deafult`属性。否则直接返回,这样就可以对于`cjs`模块的预构建后结果差异和非差异的情况都可以进行正确的返回。
...
1. `id` 收集依赖的引入模块名称
2. `exportsData` 收集依赖模块的入口文件通过`es-module-lexer`词法解析后的描述
...
对`moment`模块单做特殊处理, 因为使用的是`ESM`导出导入, 但仍使用`"require"`, 这会导致`esbuild`将它们包装为`CJS`，即使其入口看起来是`ESM`。所以还是需要当作`cjs`的方式进行处理。
...
通过循环找到对应依赖的构建描述。同时赋值在`generatedExports`变量上。
...
如果没有任何`esm`语法的导出，则认定为`cjs`语法，否则其它情况都认为`esm`的正确导入依赖方式。并没有存在混用。
...
最后向`cacheDir`中写入`_metadata.json`文件,同时返回结果。
